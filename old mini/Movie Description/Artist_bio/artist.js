import ArtistService from "../Artist_bio/ArtistService.js"
$(document).ready(function () {

    $(".body_container_Artist").html = ""
    $(".body_container_Artist").css({ 'width': '100vw', 'height': '40vw', 'border': '1px solid white', 'background-color': 'rgba(255,255,255,0.9)' })
    ...
    ArtistService.getArtistDetails()
        .then((response) => {
            ...
            console.log(artist);
            for (let a of artist) {
                if (a.Artistid.toString() == tempArtistName.toString()) {
                    console.log("hello");
                    let img = `<div class='artist_image'><img src=${a.ArtistImage}></div>`
                    let data = `<div class='artist_data'><span>${a.ArtistName}</span><table><tr><td>Also Known as:</td><th>${a.ArtistName}</th></tr>
                <tr><td>D.O.B:</td><th>${a.ArtistDob}</th></tr><tr><td>Birth City:</td><th>${a.BirthPlace}</th></tr><tr><td>Occupation:</td><th>${a.Occupation}</th></tr></table></div>`
                    ...
                    $(".artist_data").css({ 'font-size': '14px', 'color': 'white', 'position': 'absolute', 'left': '45%', 'top': '30%', })
                    $(".artist_image").css({ 'position': 'absolute', 'top': '30%', 'left': '20%', 'display': 'inline-block' })
