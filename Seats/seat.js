...
const clickEventHandler = () => {
    $(".seat").click(function () {
        ...
        if ($(this).attr("class") == "seat selected") {
            ...
            flag = true;
            seat_selected_array.push({ row: $(this).parent().attr("id"), seatNo: $(this).attr("id"), status: $(this).attr("class"),code:$(this).attr("dataId") })
            console.log(seat_selected_array);
        }
...
function DisplaySeat(time) {
    $(".cinema-seats").append(`<div class="cinema-row row-1" id="row-1" style="justify-content: center"></div>`)

    SeatService.getSeatStructure(time,"row-1").then((res) => {
        let structure = res;


        structure.forEach(seat => {
            $(".row-1").append(`<div class=${seat.Status}  id=${seat.id} dataId=${seat.SeatNo}>${seat.SeatNo} </div>`)
    ...
    SeatService.getSeatStructure(time,"row-2").then((res) => {
        let structure = res;

        structure.forEach(seat => {
            $(".row-2").append(`<div class=${seat.Status}  id=${seat.id}dataId=${seat.SeatNo}>${seat.SeatNo}</div>`)

        })

    })
    $(".cinema-seats").append(`<div class="cinema-row row-3" id="row-3" style="justify-content: center"></div>`)
    SeatService.getSeatStructure(time,"row-3").then((res) => {
        let structure = res;

        structure.forEach(seat => {
            $(".row-3").append(`<div class=${seat.Status}  id=${seat.id}dataId=${seat.SeatNo}>${seat.SeatNo}</div>`)

        })
    })
    $(".cinema-seats").append(`<div class="cinema-row row-4" id="row-4" style="justify-content: center"></div>`)
    SeatService.getSeatStructure(time,"row-4").then((res) => {
        let structure = res;

        structure.forEach(seat => {
            $(".row-4").append(`<div class=${seat.Status}  id=${seat.id}dataId=${seat.SeatNo}>${seat.SeatNo}</div>`)

        })
    })
    $(".cinema-seats").append(`<div class="cinema-row row-5" id="row-5" style="justify-content: center"></div>`)
    SeatService.getSeatStructure(time,"row-5").then((res) => {
        let structure = res;

        structure.forEach(seat => {
            $(".row-5").append(`<div class=${seat.Status}  id=${seat.id}dataId=${seat.SeatNo}>${seat.SeatNo}</div>`)
...
function updateSeat(event) {
    ...
    for (let i = 0; i < seat_selected_array.length; i++) {
        ...
        if (fillSeat.status == "seat selected") {
            ...
            SeatService.changeSeatStatus("6.00PM",seat_selected_array[i].row,seat_selected_array[i].seatNo,seat_selected_array[i].Status ).then((res) => {
                console.log("Success");
            });
